...
upbound, count = list(map(int, input().split(' ')))
upbound = int(upbound / 10)
value = [[0, 0, 0] for i in range(count + 1)]
weight = [[0, 0, 0] for i in range(count + 1)]
...
dp = [[0] * int(upbound + 1) for i in range(count + 1)]

'''可能要对upbound 容量排序，否则要添加else语句，不然会有一些情况出错'''
for i in range(1, count + 1):
    for j in range(upbound + 1):
        if j >= weight[i][0]:
            dp[i][j] = max(dp[i][j], dp[i - 1][j], dp[i - 1][j - weight[i][0]] + value[i][0])
